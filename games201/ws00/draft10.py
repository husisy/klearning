# https://github.com/ShaneFX/GAMES201
import time
import enum
import numpy as np
import taichi as ti
...
class SmokeSolver:
    def __init__(self, x, y, z):
        ...
        self.velocity_divs = ti.field(ti.f32, shape=self.res)
        self._pressures = ti.field(ti.f32, shape=self.res)
        self._new_pressures = ti.field(ti.f32, shape=self.res)
        self._dens_buffer = ti.field(ti.f32, shape=self.res)
        self._new_dens_buffer = ti.field(ti.f32, shape=self.res)
        ...
        self.r = [ti.field(ti.f32) for _ in range(self.n_mg_levels)]  # residual
        self.z = [ti.field(ti.f32) for _ in range(self.n_mg_levels)]  # M^-1 self.r
        self.x = ti.field(ti.f32)  # solution
        self.p = ti.field(ti.f32)  # conjugate gradient
        self.Ap = ti.field(ti.f32)  # matrix-vector product
        self.alpha = ti.field(ti.f32)  # step size
        self.beta = ti.field(ti.f32)  # step size
        self.sum = ti.field(ti.f32)  # storage for reductions
