...
DIM = 3
WINDOW_SIZE = 1024
rgb2hex = lambda x: x[0].astype(np.int32) * 65536 + x[1].astype(np.int32) * 256 + x[2].astype(np.int32)
...
class MolecularDynamics:
    ...
    def __init__(self, density, temperature, boxlength, dt):
        ...
        # kinetic energy
        self.ek = ti.field(ti.f32, shape=())
        # potential energy
        self.ep = ti.field(ti.f32, shape=())
        # temperature
        self.temp = ti.field(ti.f32, shape=())
        # initialize system
        # init_thermostat
        ...
        # initilize position: Place particles on a regular grid and randomize their velocities according to the temperature
        ...
        self.position.from_numpy(np.stack(np.meshgrid(*coords_1d)).reshape(DIM, -1)[:, :self.n_particles].T)
...
if __name__ == "__main__":
    ...
    md = MolecularDynamics(density, temperature, boxlength, dt=5e-3)
    ...
    i = 0
    while md.gui.running:
        for _ in range(10):
            md.integrate_thermostat()
            md.integrate_motion()
            md.integrate_thermostat()
        md.render()
        # md.render("frame%i.png" % i)
        i += 1
