# https://github.com/YuCrazing/Taichi/tree/master/fem_3d
import math
...
class Object:
    def __init__(self, filename, index_start=1):
        ...
        self.center = ti.Vector.field(self.dim, ti.f32, shape=())
        self.lowerCorner = ti.Vector.field(self.dim, ti.f32, shape=())
        self.higherCorner = ti.Vector.field(self.dim, ti.f32, shape=())
        ...
        self.neighbor_element_count = ti.field(ti.i32, shape=self.vn)
        self.node_mass = ti.field(ti.f32, shape=self.vn)
        self.element_mass = ti.field(ti.f32, shape=self.en)
        self.element_volume = ti.field(ti.f32, shape=self.en)
        self.B = ti.Matrix.field(self.dim, self.dim, dtype=ti.f32, shape=self.en) # a square matrix
        self.energy = ti.field(ti.f32, shape=(), needs_grad=True)

        # for rendering
        self.color = ti.field(ti.i32, shape=self.vn)
