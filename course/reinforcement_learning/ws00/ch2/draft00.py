...
def figure_2_2(runs=2000, time=3000):
    epsilons = [0, 0.01, 0.1]
    ...
    for eps_i, x in zip(epsilons, rewards):
        ax0.plot(x, label=r'$\epsilon$ = %.02f' % (eps_i))
    ax0.set_xlabel('steps')
    ax0.set_ylabel('average reward')
    ax0.legend()

    for eps_i, x in zip(epsilons, best_action_counts):
        ax1.plot(x, label='epsilon = %.02f' % (eps_i))
    ax1.set_xlabel('steps')
    ax1.set_ylabel('% optimal action')
    ax1.legend()
    fig.tight_layout()

    fig.savefig('../images/figure_2_2.png')
    plt.close(fig)
